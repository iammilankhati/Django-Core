...
//blog/base.html for alert message {% if messages %}
{% for message in messages %}
<div class="alert alert-{{message.tags}}">{{message}}</div> {% endfor %}
{% endif %}

//custom forms class UserRegistrationForm(UserCreationForm): email:
forms.EmailField()
...
### AAAAMZAING CHECKING AUTHENTICATION BY BUILT IN user.is_authenticated
...
### Using login_required decorator to restrict certain route
```
@login_required
def profile(request):
